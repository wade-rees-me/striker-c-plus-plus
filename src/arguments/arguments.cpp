...
Arguments::Arguments(int argc, char *argv[]) {
    for (int i = 1; i < argc; ++i) {
        ...
        } else if ((std::strcmp(argv[i], "-t") == 0 || std::strcmp(argv[i], "--number-of-threads") == 0) &&
                   i + 1 < argc) {
...
//
void Arguments::printVersion() const { std::cout << STRIKER_WHO_AM_I << ": version: " << STRIKER_VERSION << std::endl; }
