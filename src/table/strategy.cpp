...
Strategy::Strategy(const std::string &decks, const std::string &strategy, const int number_of_decks)
    : Request(), number_of_cards(number_of_decks * NUMBER_OF_CARDS_IN_DECK), SoftDouble("Soft Double"),
      HardDouble("Hard Double"), PairSplit("Pair Split"), SoftStand("Soft Stand"), HardStand("Hard Stand") {
    try {
...
void Strategy::fetchTable(const std::string &decks, const std::string &strategy) {
    ...
    strategyLoadTable(jsonResponse["soft-double"].get<const std::map<std::string, std::vector<std::string>>>(),
                      &SoftDouble);
    strategyLoadTable(jsonResponse["hard-double"].get<const std::map<std::string, std::vector<std::string>>>(),
                      &HardDouble);
    strategyLoadTable(jsonResponse["pair-split"].get<const std::map<std::string, std::vector<std::string>>>(),
                      &PairSplit);
    strategyLoadTable(jsonResponse["soft-stand"].get<const std::map<std::string, std::vector<std::string>>>(),
                      &SoftStand);
    strategyLoadTable(jsonResponse["hard-stand"].get<const std::map<std::string, std::vector<std::string>>>(),
                      &HardStand);
}
